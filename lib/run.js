...
function install_deps() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (process.platform == 'darwin') {
            ...
            let runner0 = new toolrunner.ToolRunner('sudo', ['apt-get', 'update']);
            runner0.exec();
            let runner_ = new toolrunner.ToolRunner('sudo', ['apt-get', 'install', 'ruby-full', 'build-essential', 'libpq-dev']);
           yield runner_.exec();
        }
        ...
        let runner = null;
        if (process.platform == 'darwin') {
            runner = new toolrunner.ToolRunner('gem', ['install', 'bundler:1.17.2']);
        } else {
            runner = new toolrunner.ToolRunner('sudo', ['gem', 'install', 'bundler:1.17.2']);
        }
        yield runner.exec();

        let runner0 = null;
        if (process.platform == 'darwin') {
            runner0 = new toolrunner.ToolRunner('gem',
                ['install', 'capistrano', 'capistrano-rails']);
        } else {
            runner0 = new toolrunner.ToolRunner('sudo',
